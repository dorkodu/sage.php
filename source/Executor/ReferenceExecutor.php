...
use Sage\Language\AST\DocumentNode;
use Sage\Language\AST\FieldNode;
use Sage\Language\AST\FragmentDefinitionNode;
use Sage\Language\AST\FragmentSpreadNode;
use Sage\Language\AST\InlineFragmentNode;
use Sage\Language\AST\Node;
use Sage\Language\AST\OperationDefinitionNode;
use Sage\Language\AST\SelectionNode;
use Sage\Language\AST\SelectionSetNode;
use Sage\Type\Definition\AbstractType;
use Sage\Type\Definition\Directive;
use Sage\Type\Definition\FieldDefinition;
use Sage\Type\Definition\InterfaceType;
use Sage\Type\Definition\LeafType;
...
class ReferenceExecutor implements ExecutorImplementation
{
  ...
  public function doExecute(): Promise
  {
    ...
    // the "Response" section of the Sage specification.
    //
    // If errors are encountered while executing a Sage field, only that
   ...
   * value of the type by calling the `serialize` method of Sage type
   * definition.
   *
      ...
      throw new InvariantViolation(
        sprintf(
          'Schema must contain unique named types but contains multiple types named "%s". %s ' .
            '(see http://webonyx.github.io/Sage-php/type-system/#type-registry).',
          $returnType,
          $hint
        )
      );
    ...
    if ($this->exeContext->promiseAdapter->isThenable($value)) {
      ...
      if (!$promise instanceof Promise) {
        throw new InvariantViolation(sprintf(
          '%s::convertThenable is expected to return instance of Sage\Executor\Promise\Promise, got: %s',
  ...
  {
    $itemType = $returnType->wrappedType();
    Utils::invariant(
  ...
  private function defaultTypeResolver($value, $contextValue, ResolveInfo $info, AbstractType $abstractType)
  {
    ...
    if ($abstractType instanceof InterfaceType && $info->schema->getConfig()->typeLoader !== null) {
      Warning::warnOnce(
        sprintf(
          'Sage Interface Type `%s` returned `null` from its `resolveType` function ' .
   ...
   *
   * @see https://github.com/webonyx/Sage-php/issues/59
   *
