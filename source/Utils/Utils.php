...
class Utils
{
     ...
     */
    public static function error($test, $message = '')
    {
        if (!$test) {
            if (func_num_args() > 2) {
                $args = func_get_args();
    ...
    {
        self::error(is_string($name), 'Expected name to be string');
        self::error(!empty($name), 'Expected name to be non-empty string');
